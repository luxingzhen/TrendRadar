...
def load_config():
    ...
    # PushDeer配置
    config["PUSHDEER_SERVER_URL"] = os.environ.get(
        "PUSHDEER_SERVER_URL", "https://api2.pushdeer.com"
    ).strip() or webhooks.get("pushdeer_server_url", "https://api2.pushdeer.com")
    config["PUSHDEER_PUSHKEY"] = os.environ.get(
        "PUSHDEER_PUSHKEY", ""
    ).strip() or webhooks.get("pushdeer_pushkey", "")
    ...
    if config["PUSHDEER_SERVER_URL"] and config["PUSHDEER_PUSHKEY"]:
        server_source = (
            "环境变量" if os.environ.get("PUSHDEER_SERVER_URL") else "配置文件"
        )
        notification_sources.append(f"PushDeer({server_source})")
...
def format_title_for_platform(
    ...
    """统一的标题格式化方法"""
    # PushDeer 使用与 ntfy 相同的 markdown 格式
    if platform == "pushdeer":
        platform = "ntfy"
...
def split_content_into_batches(
    ...
    if max_bytes is None:
        if format_type == "dingtalk":
            ...
            max_bytes = 3800
        elif format_type == "pushdeer":
            max_bytes = 30000
        else:
            max_bytes = CONFIG.get("MESSAGE_BATCH_SIZE", 4000)

    # PushDeer 使用与 ntfy 相同的 markdown 格式，仅限制不同
    if format_type == "pushdeer":
        format_type = "ntfy"
...
def send_to_notifications(
    ...
    pushdeer_server_url = CONFIG.get(
        "PUSHDEER_SERVER_URL", "https://api2.pushdeer.com"
    )
    pushdeer_pushkey = CONFIG.get("PUSHDEER_PUSHKEY", "")
    ...
    # 发送到 PushDeer
    if pushdeer_server_url and pushdeer_pushkey:
        results["pushdeer"] = send_to_pushdeer(
            pushdeer_server_url,
            pushdeer_pushkey,
            report_data,
            report_type,
...
def send_to_pushdeer(
    ...
    """发送到 PushDeer（支持分批发送，遵守 PushDeer 单条消息大小限制）"""
    # 避免 HTTP header 编码问题
    report_type_en_map = {
        "当日汇总": "Daily Summary",
        "当前榜单汇总": "Current Ranking",
    ...
    # 获取分批内容，使用 PushDeer 专用的 30KB 限制
    batches = split_content_into_batches(
        report_data, "pushdeer", update_info, max_bytes=30000, mode=mode
    )

    total_batches = len(batches)
    print(f"PushDeer消息分为 {total_batches} 批次发送 [{report_type}]")

    # 逐批发送（PushDeer 客户端按时间顺序显示，正序发送即可）
    success_count = 0
    for idx, batch_content in enumerate(batches, 1):
        batch_size = len(batch_content.encode("utf-8"))
        print(
            f"发送PushDeer第 {idx}/{total_batches} 批次，大小：{batch_size} 字节 [{report_type}]"
        )

        # 检查消息大小，确保不超过 PushDeer 限制
        if batch_size > 30000:
            print(f"警告：PushDeer第 {idx} 批次消息过大（{batch_size} 字节），可能被拒绝")

        # 标题
        text = report_type_en
        if total_batches > 1:
            text = f"{report_type_en} ({idx}/{total_batches})"
        ...
        try:
            ...
            # PushDeer 返回 JSON: {"code":0,"content":{...}} 表示成功
            resp_code = None
            try:
            ...
            if response.status_code == 200 and resp_code == 0:
                print(f"PushDeer第 {idx}/{total_batches} 批次发送成功 [{report_type}]")
                success_count += 1
                if idx < total_batches:
                    # PushDeer 对频率有限制，批次间稍作间隔
                    time.sleep(1)
            else:
                print(
                    f"PushDeer第 {idx}/{total_batches} 批次发送失败 [{report_type}]，状态码：{response.status_code}，响应：{response.text}"
                )

        except requests.exceptions.ConnectTimeout:
            print(f"PushDeer第 {idx}/{total_batches} 批次连接超时 [{report_type}]")
        except requests.exceptions.ReadTimeout:
            print(f"PushDeer第 {idx}/{total_batches} 批次读取超时 [{report_type}]")
        except requests.exceptions.ConnectionError as e:
            print(f"PushDeer第 {idx}/{total_batches} 批次连接错误 [{report_type}]：{e}")
        except Exception as e:
            print(f"PushDeer第 {idx}/{total_batches} 批次发送异常 [{report_type}]：{e}")

    # 判断整体发送是否成功
    if success_count == total_batches:
        print(f"PushDeer所有 {total_batches} 批次发送完成 [{report_type}]")
        return True
    elif success_count > 0:
        print(f"PushDeer部分发送成功：{success_count}/{total_batches} 批次 [{report_type}]")
        return True  # 部分成功也视为成功
    else:
        print(f"PushDeer发送完全失败 [{report_type}]")
        return False
    ...
    def _has_notification_configured(self) -> bool:
        ...
        return any(
            [
                ...
                (
                    CONFIG.get("PUSHDEER_SERVER_URL")
                    and CONFIG.get("PUSHDEER_PUSHKEY")
                ),
            ]
        )
