...
[![PushDeer通知](https://img.shields.io/badge/PushDeer-通知-00D4AA?style=flat-square)](https://www.pushdeer.com/)
...
支持**企业微信**(+ 微信推送方案)、**飞书**、**钉钉**、**Telegram**、**邮件**、**ntfy**、**PushDeer**，消息直达手机和邮箱
...
2. **设置 GitHub Secrets（选择你需要的平台）**:
   ...
   **相关链接：**
   ...
   <details>
   <summary> <strong>👉 PushDeer 推送</strong>（开源，支持自托管，markdown 富文本）</summary>
   <br>
   ...
   **特点**：
   - ✅ 无需自建服务器，免费使用
   - ✅ 支持 markdown 富文本消息
   - ✅ 多端客户端：iOS / Android / 桌面 / 微信
   - ⚠️ 公共服务有频率限制，正常使用足够

   **快速开始：**

   1. **获取 PushDeer 的 pushkey**：
      - 手机/电脑访问 [PushDeer 官网](https://www.pushdeer.com/)
      - 使用 Apple ID 或手机号登录并注册设备
      - 在客户端设置页面复制你的 **PushKey**（形如 `PDUxxxxxxxxxxxxxxxxxxxxxxxx`）

   2. **配置 TrendRadar**：
      - 在 [config/config.yaml](config/config.yaml) 中填写：
        ```yaml
        notification:
          webhooks:
            pushdeer_server_url: "https://api2.pushdeer.com"  # 公共服务，可改为自托管地址
            pushdeer_pushkey: "PDUxxxxxxxxxxxxxxxxxxxxxxxx"   # 你的 PushKey
        ```
      - 或使用环境变量 / Docker `.env`：
        ```
        PUSHDEER_SERVER_URL=https://api2.pushdeer.com
        PUSHDEER_PUSHKEY=PDUxxxxxxxxxxxxxxxxxxxxxxxx
        ```
      - **GitHub Actions 部署**：在仓库 `Settings` > `Secrets and variables` > `Actions` 中新建 Secret：
        - `PUSHDEER_SERVER_URL`：填写 `https://api2.pushdeer.com`（或自托管地址）
        - `PUSHDEER_PUSHKEY`：填写你的 PushKey

   3. **测试**：
      ```bash
      curl -X POST https://api2.pushdeer.com/message/push \
        -d "pushkey=PDUxxxxxxxxxxxxxxxxxxxxxxxx" \
        -d "text=测试消息" \
        -d "desp=TrendRadar 推送测试" \
        -d "type=markdown"
      ```
   ...
   **Docker 一键部署**：
   ...
   docker run -d \
     --name pushdeer \
     -p 8843:80 \
     -v /var/lib/pushdeer:/data \
     easychen/pushdeer
   ```

   **配置 TrendRadar**：
   ```yaml
   PUSHDEER_SERVER_URL: https://pushdeer.yourdomain.com  # 自托管服务器地址
   PUSHDEER_PUSHKEY: PDUxxxxxxxxxxxxxxxxxxxxxxxx
   ```
   ...
   **相关链接：**
   - [PushDeer 官网](https://www.pushdeer.com/)
   - [PushDeer GitHub 仓库](https://github.com/easychen/pushdeer)
...
docker run -d --name trend-radar \
  ...
  -e EMAIL_TO="收件人邮箱" \
  -e PUSHDEER_SERVER_URL="https://api2.pushdeer.com" \
  -e PUSHDEER_PUSHKEY="你的PushDeer pushkey" \
  -e CRON_SCHEDULE="*/30 * * * *" \
  -e RUN_MODE="cron" \
